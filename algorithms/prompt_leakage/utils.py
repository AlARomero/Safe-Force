...
from algorithms.prompt_leakage.model_anthropic import Anthropic_Claude
from algorithms.prompt_leakage.model_google import Google_Gemini, Google_PaLM
from algorithms.prompt_leakage.model_cohere import CohereModel
from algorithms.prompt_leakage.model_openai_wrapper import OpenAILLM
from algorithms.prompt_leakage.model_ollama import OllamaModel
import pdb
...
def load_domain_examples(domain):
    ...
    if domain == 'news':
        dataset = json.load(open('algorithms/prompt_leakage/data/news/newsqa_triplets_trunc_200.json','r'))
        ic_examples = open('algorithms/prompt_leakage/data/news/in_context_egs.json','r').readlines()

    elif domain == 'medical':
        dataset = json.load(open('algorithms/prompt_leakage/data/medical/medicalqa_bioasq_triplets_trunc_200.json','r'))
        ic_examples = open('algorithms/prompt_leakage/data/medical/in_context_egs.json','r').readlines()

    elif domain == 'legal':
        dataset = json.load(open('algorithms/prompt_leakage/data/legal/billqa_triplets_trunc_200.json','r'))
        ic_examples = open('algorithms/prompt_leakage/data/legal/in_context_egs.json','r').readlines()

    elif domain == 'finance':
        dataset = json.load(open('algorithms/prompt_leakage/data/finance/finqa_triplets_trunc_200.json','r'))
        ic_examples = open('algorithms/prompt_leakage/data/finance/in_context_egs.json','r').readlines()

    else:
        raise ArgumentError(None, f"Bad domain: {domain}")
...
def load_model(model_name):
    ...
    if 'claude' in model_name:
        ...
        llm = OllamaModel(model_name)    # for all open source models use OLLAMA
